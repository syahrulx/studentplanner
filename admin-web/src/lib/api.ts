...
export async function upsertWhatsNewPrompt(
  row: Pick<WhatsNewPromptRow, 'is_active' | 'version_name' | 'title' | 'content'> & { id?: string },
): Promise<WhatsNewPromptRow> {
  // If activating this row, deactivate all others first
  if (row.is_active) {
...
export async function deleteWhatsNewPrompt(id: string): Promise<void> {
  const { error } = await supabase
    .from('whats_new_prompts')
    .delete()
    .eq('id', id);
...
export async function getOpenAiCreditUsage() {
  const headers = await adminInvokeHeaders();
