...
- `supabase/functions/_shared/studyExtract.ts` — study-structured PDF extraction shared by `ai_pdf_extract` and `generate_flashcards`. Previously any PDF with a text layer (every PowerPoint/Word export) returned flat merged text from `unpdf` before any model ran: slide titles ran into bullets, tables collapsed, diagrams vanished. Now: per-page extraction; repeated headers/footers stripped; typed decks restructured into study Markdown (`##` per topic, `**Term**: definition`, tables kept) by the fast OpenAI tier in parallel chunks; diagram-heavy decks (low text per page) routed to Gemini with a prompt that describes every figure as `> [Figure] ...`; Gemini output cap raised 8192 to 32768. Raw text is only ever a fallback, so no path is worse than before. Existing notes keep their cached flat text until re-extracted.
