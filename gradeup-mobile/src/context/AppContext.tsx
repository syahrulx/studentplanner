...
export function AppProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    const sub = RNAppState.addEventListener('change', (state) => {
      ...
      void supabase.auth.getSession().then(({ data: { session } }) => {
        const uid = session?.user?.id;
        if (!uid) return;
        rescheduleAttendanceNotifications(uid, entries, { academicCalendar: academicCalendarRef.current }).catch(() => {});
      });
    });
    return () => sub.remove();
  }, []);
  ...
  const updateTimetableEntry = useCallback(
    async (
      ...
      rescheduleAttendanceNotifications(uid, mergedAfterUpdate, { academicCalendar: academicCalendarRef.current }).catch(() => {});
    },
    [],
  );
  ...
  const addTimetableEntry = useCallback(async (entry: TimetableEntry) => {
    ...
    rescheduleAttendanceNotifications(uid, merged, { academicCalendar: academicCalendarRef.current }).catch(() => {});
  }, []);
