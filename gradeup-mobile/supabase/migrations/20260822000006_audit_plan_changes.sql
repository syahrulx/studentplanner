-- subscription_reconciliation_audit was declared in 20260811000001 but nothing
-- ever wrote to it, so plan flips (like a wrong demotion) left no trail to
-- investigate. Write the audit from inside recompute_subscription_access — the
-- single choke point every caller goes through (RevenueCat webhook, Curlec
-- webhook, admin actions, the web refund endpoint, and the daily sweep) — so
-- every plan change is recorded with what won and why.
-- Otherwise identical to 20260822000002 (which added 'halted' support).
...
begin
  ...
  -- Record every effective-plan change; a failed audit insert must never
  -- roll back the recompute itself.
  begin
    if found and v_selected.plan is distinct from v_previous_plan then
      insert into public.subscription_reconciliation_audit
        (user_id, old_plan, new_plan, reason, source)
      values (
        p_user_id,
        v_previous_plan,
        v_selected.plan,
        'winner: ' || v_selected.store || ' (' || v_selected.status || ')',
        'recompute_subscription_access'
      );
    elsif not found and v_previous_plan is distinct from 'free' then
      insert into public.subscription_reconciliation_audit
        (user_id, old_plan, new_plan, reason, source)
      values (
        p_user_id,
        v_previous_plan,
        'free',
        'no active entitlement from any provider',
        'recompute_subscription_access'
      );
    end if;
  exception when others then
    raise warning 'recompute_subscription_access: audit insert failed for %: %', p_user_id, sqlerrm;
  end;
