...
function inferAcademicLevelFromOfferLabel(label: string): AcademicLevel {
  ...
  if (
    s.includes("bachelor") ||
    s.includes("sarjana muda") ||
    s.includes("first degree")
  )
    return "Bachelor";
  if (s.includes("phd") || s.includes("doktor") || s.includes("doctorate"))
    return "PhD";
...
export default function AcademicCalendarScreen() {
  ...
  const [monthCursor, setMonthCursor] = useState<Date>(() => {
    ...
    const d = /^\d{4}-\d{2}-\d{2}$/.test(base)
      ? new Date(`${base}T00:00:00`)
      : new Date();
    return new Date(d.getFullYear(), d.getMonth(), 1);
  });
  const [filter, setFilter] = useState<
    "all" | "registration" | "lecture" | "exam" | "break"
  >("all");
  ...
  useFocusEffect(
    useCallback(() => {
      ...
      activeDateIsInAcademicCalendar,
      todayISO,
      todayMonthCursor,
    ]),
  );
    ...
    void (async () => {
      try {
        ...
        const match = list.find(
          (o) => o.startDate === curStart && o.semesterLabel === curLabel,
        );
  ...
  const onHeaderAlignPress = useCallback(() => {
    if (!academicCalendar?.startDate) {
      Alert.alert(
        "No calendar",
        "Save your semester configuration first so dates are available.",
      );
      return;
    }
  ...
  const saveWeekAlign = useCallback(async () => {
    ...
    try {
      ...
      Alert.alert(
        "Error",
        e instanceof Error ? e.message : "Could not save week alignment",
      );
    } finally {
      setAlignBusy(false);
    }
  ...
  const changeUniversity = useCallback(async () => {
    ...
    try {
      ...
      Alert.alert(
        "Error",
        e instanceof Error ? e.message : "Could not change university",
      );
    } finally {
      setCfgBusy(false);
    }
  ...
  const onConfirmUniversity = useCallback(
    async (item: UniversityItem) => {
      ...
      try {
        ...
        if (item.id === "uitm") {
          const grp: "A" | "B" =
            user.academicLevel === "Foundation" ? "A" : "B";
          const official = await fetchUitmAcademicCalendar(grp, {
            targetDateISO: today,
            variant: "auto",
          });
          if (official?.startDate && official?.endDate) {
            ...
            Alert.alert(
              "Calendar sync",
              "Could not load UiTM HEA calendar. Check your connection and try Semester configuration.",
            );
          }
        } else {
        ...
        Alert.alert(
          "Error",
          e instanceof Error ? e.message : "Could not save university",
        );
      } finally {
        setCfgBusy(false);
      }
  ...
  const saveConfiguration = useCallback(async () => {
    ...
    if (!user.universityId) {
      Alert.alert(
        "University required",
        "Please confirm your university first.",
      );
      return;
    }
    try {
      ...
      if (uniId === "uitm") {
        await updateProfile({
          ...
          studyMode,
          ...(hint?.currentSemester != null
            ? { currentSemester: hint.currentSemester }
            : {}),
          heaTermCode: null,
          studentId: sid,
        });
      ...
      const selected =
        adminOffers.find((o) => o.id === cfgSelectedOfferId) ??
        (adminOffers.length > 0 ? adminOffers[0] : null);
      await updateProfile({
        studyMode,
        ...(hint?.currentSemester != null
          ? { currentSemester: hint.currentSemester }
  ...
  const filterMatches = useCallback(
    (t: string): boolean => {
      ...
      if (filter === "break")
        return type === "break" || type === "special_break";
      return type === "exam" || type === "test" || type === "revision";
    },
    [filter],
  );

  const periodsForDay = useCallback(
    (iso: string) => {
  ...
  const periodsForDayAll = useCallback(
    (iso: string) => {
      const ss = String(iso).slice(0, 10);
      return periods.filter((p) => {
        const a = String(p.startDate).slice(0, 10);
  ...
  const markerColorForType = useCallback(
    (t: string): string => {
      const type = String(t || "");
  ...
  const splitLabel = useCallback((label: string) => {
    const parts = String(label || "")
      ...
      .filter(Boolean);
    if (parts.length >= 2)
      return { tags: [parts[0]], text: parts.slice(1).join(" • ") };
  ...
  const tagColor = useCallback(
    (tag: string): string => {
      ...
      if (t.includes("bachelor") || t.includes("sarjana muda"))
        return "#f97316";
  ...
  return (
    <View style={s.safe}>
          ...
          {[
            ...
            <Pressable
              ...
              style={[
                ...
                {
                  borderColor:
                    filter === (opt.id as any) ? theme.primary : theme.border,
                  backgroundColor:
                    filter === (opt.id as any) ? theme.primary : "transparent",
                },
              ]}
        ...
        <View style={s.grid}>
          {monthGrid.map((d) => {
            ...
            const cats = Array.from(
              new Set(hits.map((h) => categoryForType(h.type))),
            ).filter((c) => c !== "other");
            const dominant =
              hits.length > 0 ? primaryCategory(hits as any) : "other";
            const bg =
              filter !== "all"
                ? hits.length > 0
                  ? markerBgForType(filter === "break" ? "break" : filter)
                  : "transparent"
                : dominant === "other"
                  ? "transparent"
                  : markerBgForType(dominant);
            return (
              <Pressable
                key={d.iso}
                style={[
                  ...
                  <View style={s.dotRow}>
                    {cats.slice(0, 4).map((c) => (
                      <View
                        ...
                        style={[
                          s.dot,
                          { backgroundColor: markerColorForType(c) },
                        ]}
                      />
                    ))}
                  </View>
        ...
        {selectedDayISO ? (
          ...
          >
            ...
            {selectedDayItems.length === 0 ? (
              <Text style={[s.empty, { marginTop: 8 }]}>
                  ...
                  <View key={g.id} style={{ marginTop: 12 }}>
                    ...
                    >
                      <View
                        style={[
                          s.catDot,
                          { backgroundColor: markerColorForType(g.id) },
                        ]}
                      />
                      ...
                      return (
                        <View key={`${g.id}-${idx}`} style={s.eventRow}>
                          <View
                            style={[
                              s.eventDot,
                              { backgroundColor: markerColorForType(p.type) },
                            ]}
                          />
                          <View style={{ flex: 1, minWidth: 0 }}>
                            {tags.length > 0 ? (
                            ...
                            <Text
                              style={[s.eventTitle, { color: theme.text }]}
                              numberOfLines={4}
                            >
                              {text || String(p.type)}
                            </Text>
        ...
        <View style={s.modalBackdrop}>
          ...
          >
            <ScrollView
              style={[
                s.modalCard,
                { backgroundColor: theme.card, maxHeight: modalMaxH },
              ]}
            ...
            >
              ...
              {user.universityId === "uitm" ? (
                <>
                  ...
                  <Text style={[s.modalSub, { color: theme.textSecondary }]}>
                    HEA shows a starred (*) date range for
                  ...
                  >
                    {(
                      ...
                      ] as { id: UitmCalendarVariant; label: string }[]
                    ).map((opt) => (
                      <Pressable
                        key={opt.id}
              ...
              {user.universityId === "uitm" ? (
                <>
                  <Text style={[s.modalSub, { color: theme.textSecondary }]}>
                    UiTM uses HEA groups: Foundation → Group A; other levels →
                    Group B.
                  </Text>
                  {(
                    [
                    ...
                    >
                      ...
                      />
                      <Text style={[s.optText, { color: theme.text }]}>
                        {lvl}
                      </Text>
                      <Text
                        style={[s.optBadge, { color: theme.textSecondary }]}
                      >
                ...
                <>
                  ...
                  >
                    <Text style={[s.saveBtnText, { color: theme.text }]}>
                      Can't find yours? Add it yourself
                    </Text>
                  </Pressable>
                </>
              )}
              ...
              <Text style={[s.modalSub, { color: theme.textSecondary }]}>
                If the week shown in the header does not match the week you are
                actually in, pick your real teaching week. This does not change
                lecture or exam dates — only which week number we show you.
              </Text>
              <Pressable
                style={[
                  s.saveBtn,
                  ...
                  (!academicCalendar?.startDate || cfgBusy) && {
                    opacity: 0.45,
                  },
                ]}
              ...
              >
                {cfgBusy ? (
                  ...
                  <Text style={[s.saveBtnText, { color: theme.textInverse }]}>
                    Save
                  </Text>
                )}
              </Pressable>
      ...
      >
        ...
        >
          <Text style={[s.modalTitle, { color: theme.text, fontSize: 20 }]}>
            Confirm your university
          </Text>
          <Text
            style={[s.modalSub, { color: theme.textSecondary, marginTop: 8 }]}
          >
            We load your academic calendar from official sources: UiTM uses HEA;
            other universities use calendars published by your app administrator
            (for example UM).
          </Text>
          <TextInput
            value={uniSearch}
      ...
      <Modal
        visible={weekAlignOpen}
        transparent
        animationType="fade"
        onRequestClose={() => setWeekAlignOpen(false)}
      >
        <View style={s.modalBackdrop}>
          <Pressable
            style={StyleSheet.absoluteFill}
          ...
          >
            ...
            >
              <View style={{ flex: 1, minWidth: 0 }}>
                <Text
                  style={[s.modalTitle, { color: theme.text }]}
                  numberOfLines={1}
                >
                  Align with current week
                </Text>
